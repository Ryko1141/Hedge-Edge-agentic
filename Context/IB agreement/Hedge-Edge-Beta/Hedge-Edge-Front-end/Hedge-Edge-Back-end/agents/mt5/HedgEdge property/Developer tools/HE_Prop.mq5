...
int OnInit()
{
   ...
   if(!dllAvailable)
   {
      ...
      if(!InpDevMode)
      {
         ...
         if(!ValidateLicenseViaWebRequest())
   ...
   {
      if(StringLen(InpLicenseKey) == 0 && !InpDevMode)
      {
         g_statusMessage = "ERROR: License Key is required";
         UpdateComment();
         return INIT_PARAMETERS_INCORRECT;
...
void OnTick()
{
   ...
   {
      if(g_dllLoaded) ValidateLicenseWithDLL();
      else ValidateLicenseViaWebRequest();
   }
}
...
bool ValidateLicenseWithDLL()
{
   ...
   int result = ValidateLicense(InpLicenseKey,
